...
def menu(self, context):
	if (IsMenuEnable(__name__.split('.')[-1])):
		...
		op = row.operator('object.make_link_transform', icon='PLUGIN', text="位置コピー")
		op.copy_location, op.copy_rotation, op.copy_scale = True, False, False
		op = row.operator('object.make_link_transform', icon='PLUGIN', text="回転コピー")
		op.copy_location, op.copy_rotation, op.copy_scale = False, True, False
		op = row.operator('object.make_link_transform', icon='PLUGIN', text="拡縮コピー")
